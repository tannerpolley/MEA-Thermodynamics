...
def require(path: Path) -> Path:
    if not path.exists():
        raise RuntimeError(f"Missing {path}. Run analyses\\six_species_legacy\\scripts\\generate_data.py first.")
    return path
...
def render_pressure() -> Path:
    ...
    fig.tight_layout()
    png = PRESSURE_DIR / "legacy_pcsaft_jou_recomputed_fit.png"
    svg = PRESSURE_DIR / "legacy_pcsaft_jou_recomputed_fit.svg"
    fig.savefig(png, dpi=300, bbox_inches="tight")
    fig.savefig(svg, bbox_inches="tight")
    plt.close(fig)
    write_mpl_sidecar(PRESSURE_DIR / "legacy_pcsaft_jou_recomputed_fit.mpl.yaml", png_name=png.name, svg_name=svg.name, title=title, description=description)
    return png
...
def render_speciation() -> Path:
    ...
    fig.tight_layout()
    png = SPECIATION_DIR / "speciation.png"
    svg = SPECIATION_DIR / "speciation.svg"
    fig.savefig(png, dpi=300, bbox_inches="tight")
    fig.savefig(svg, bbox_inches="tight")
    plt.close(fig)
    write_mpl_sidecar(SPECIATION_DIR / "speciation.mpl.yaml", png_name=png.name, svg_name=svg.name, title=title, description=description)
    return png
