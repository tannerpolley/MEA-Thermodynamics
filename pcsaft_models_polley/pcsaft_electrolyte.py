...
def pcsaft_vaporP(p_guess, x, m, s, e, t, **kwargs):
    ...
    result = minimize(vaporPfit, p_guess, args=(x, m, s, e, t, kwargs), tol=1e-10, method='Nelder-Mead',
                      options={'maxiter': 100})
...
def pcsaft_bubbleP(p_guess, xv_guess, x, m, s, e, t, **kwargs):
    ...
    """
    check_input(x, 'guess pressure', p_guess, 'temperature', t)
    result = minimize(bubblePfit, p_guess, args=(xv_guess, x, m, s, e, t, kwargs), tol=1e-10, method='Nelder-Mead',
                      options={'maxiter': 100})
    ...
    if not (
            'z' in kwargs):  # Check that the mixture does not contain electrolytes. For electrolytes, a different equilibrium criterion should be used.
...
def pcsaft_Hvap(p_guess, x, m, s, e, t, **kwargs):
    ...
    """
    check_input(x, 'guess pressure', p_guess, 'temperature', t)
    Pvap = minimize(vaporPfit, p_guess, args=(x, m, s, e, t, kwargs), tol=1e-10, method='Nelder-Mead',
...
def pcsaft_PTz(p_guess, x_guess, beta_guess, mol, vol, x_total, m, s, e, t, **kwargs):
    ...
    result = minimize(PTzfit, p_guess, args=(x_guess, beta_guess, mol, vol, x_total, m, s, e, t, kwargs), tol=1e-10,
...
def pcsaft_den(x, m, s, e, t, p, phase='liq', **kwargs):
    ...
    rho = root(denfit, rho_guess, args=(x, m, s, e, t, p, kwargs)).x[0]
...
def pcsaft_fugcoef(x, m, s, e, t, rho, k_ij=None, e_assoc=None, vol_a=None, dipm=None, \
    ...
    for i in range(ncomp):
        ...
        # print(i, x * m * (e_ij[i, :] / t) * s_ij[i, :] ** 3)
        # print(i, np.sum(x * m * (e_ij[i, :] / t) * s_ij[i, :] ** 3))
...
def pcsaft_ares(x, m, s, e, t, rho, k_ij=None, e_assoc=None, vol_a=None, dipm=None, \
    ...
    # print(f'hc: {ares_hc:.4f}, disp: {ares_disp:.4f}, polar: {ares_polar:.4f}, assoc: {ares_assoc:.4f}, ion: {ares_ion:.4f}')
...
def XA_find(XA_guess, ncomp, delta_ij, den, x):
    ...
    for i in range(ncomp):
        summ2 = 0 * summ2
        for j in range(ncomp):
            # print(XA_guess[j, :], delta_ij[i, j]*ABmatrix)
        ...
        if prop[i] == 0:
            if molden[i] < 900:
                error[i] = (pcsaft_den(x, np.asarray([params[0]]), np.asarray([params[1]]), np.asarray([params[2]]),
                                       T[i], P[i], phase='vap', **kwargs) - molden[i]) / molden[i] * 100
            else:
                error[i] = (pcsaft_den(x, np.asarray([params[0]]), np.asarray([params[1]]), np.asarray([params[2]]),
                                       T[i], P[i], **kwargs) - molden[i]) / molden[i] * 100
        elif prop[i] == 1:
