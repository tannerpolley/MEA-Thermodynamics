...
def require(path: Path) -> Path:
    if not path.exists():
        raise RuntimeError(f"Missing {path}. Run analyses\\epcsaft_neutral_parity\\scripts\\generate_data.py first.")
    return path
...
def main() -> int:
    ...
    fig.tight_layout()
    png = OUT_DIR / "epcsaft_neutral_pcsaft_parity.png"
    svg = OUT_DIR / "epcsaft_neutral_pcsaft_parity.svg"
    fig.savefig(png, dpi=300, bbox_inches="tight")
    fig.savefig(svg, bbox_inches="tight")
    plt.close(fig)
    write_mpl_sidecar(OUT_DIR / "epcsaft_neutral_pcsaft_parity.mpl.yaml", png_name=png.name, svg_name=svg.name, title=title, description=description)
    summary_json = {
