from __future__ import annotations

import argparse
import json
import shutil
import sys
import time
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[4]
SRC_ROOT = REPO_ROOT / "src"
if str(SRC_ROOT) not in sys.path:
    sys.path.insert(0, str(SRC_ROOT))

from MEA.epcsaft_ionic.approval_check import write_global_regression_approval
from MEA.epcsaft_ionic.global_regression import GLOBAL_RESULTS_DIR, attempt_global_regression, write_global_artifacts
from MEA.epcsaft_ionic.preregistration import load_preregistration, validate_preregistration


def main() -> int:
    parser = argparse.ArgumentParser(description="Attempt coupled pressure/speciation regression for full-ionic MEA ePC-SAFT.")
    parser.add_argument("--preregistration", type=Path, default=None)
    parser.add_argument("--max-pressure-records", type=int, default=None)
    parser.add_argument("--max-speciation-records", type=int, default=None)
    parser.add_argument("--max-nfev", type=int, default=None)
    parser.add_argument("--output-label", type=str, default=None)
    parser.add_argument("--pressure-weight", type=float, default=None)
    parser.add_argument("--speciation-weight", type=float, default=None)
    parser.add_argument("--regularization-scale", type=float, default=None)
    parser.add_argument("--promote", action="store_true")
    parser.add_argument("--verbose", action="store_true")
    args = parser.parse_args()

    validated = None
    if args.preregistration is not None:
        mutable_overrides = {
            "max_pressure_records": args.max_pressure_records,
            "max_speciation_records": args.max_speciation_records,
            "max_nfev": args.max_nfev,
            "pressure_weight": args.pressure_weight,
            "speciation_weight": args.speciation_weight,
            "regularization_scale": args.regularization_scale,
        }
        changed = sorted(key for key, value in mutable_overrides.items() if value is not None)
        if changed:
            parser.error("preregistered runs reject mutable CLI overrides: " + ", ".join(changed))
        if args.output_label != "final_candidate" or not args.promote:
            parser.error("the frozen final-fit command requires --output-label final_candidate --promote")
        validated = validate_preregistration(load_preregistration(args.preregistration))
        max_pressure_records = validated.target_counts["pressure"]
        max_speciation_records = validated.target_counts["speciation"]
        max_nfev = validated.max_iterations
        pressure_weight = validated.pressure_weight
        speciation_weight = validated.speciation_weight
        regularization_scale = validated.regularization_scale
    else:
        if args.promote:
            parser.error("--promote requires a validated --preregistration file")
        max_pressure_records = args.max_pressure_records
        max_speciation_records = args.max_speciation_records
        max_nfev = args.max_nfev if args.max_nfev is not None else 0
        pressure_weight = args.pressure_weight if args.pressure_weight is not None else 1.0
        speciation_weight = args.speciation_weight if args.speciation_weight is not None else 1.0
        regularization_scale = args.regularization_scale if args.regularization_scale is not None else 0.003

    started = time.monotonic()
    payload = attempt_global_regression(
        max_pressure_records=max_pressure_records,
        max_speciation_records=max_speciation_records,
        max_nfev=max_nfev,
        pressure_weight=pressure_weight,
        speciation_weight=speciation_weight,
        regularization_scale=regularization_scale,
        execution_authorized=validated is not None,
        preregistration=(
            {
                "sha256": validated.sha256,
                "payload": validated.payload,
                "wall_time_ceiling_seconds": validated.wall_time_ceiling_seconds,
            }
            if validated is not None
            else None
        ),
        verbose=args.verbose,
    )
    if validated is not None:
        payload["preregistration"]["wall_time_seconds"] = time.monotonic() - started
    output_dir = GLOBAL_RESULTS_DIR.parent / "runs" / "global_regression" / (args.output_label or "smoke")
    summary = write_global_artifacts(payload, output_dir)
    approval = write_global_regression_approval(
        summary,
        output_dir,
        require_preregistration=args.promote,
        expected_preregistration_sha256=validated.sha256 if validated is not None else None,
    )
    if args.promote:
        if not approval["approved"]:
            print(json.dumps(approval, indent=2, sort_keys=True))
            print(json.dumps({"promotion": "rejected", "candidate_dir": str(output_dir)}, indent=2))
            return 2
        if GLOBAL_RESULTS_DIR.exists():
            shutil.rmtree(GLOBAL_RESULTS_DIR)
        shutil.copytree(output_dir, GLOBAL_RESULTS_DIR)
        output_dir = GLOBAL_RESULTS_DIR
    print(json.dumps(summary["optimizer"], indent=2))
    print(
        json.dumps(
            {
                "completion_status": summary["completion_status"],
                "selected_parameter_set": summary["selected_parameter_set"],
                "promotion_approval": approval["decision"],
                "output_dir": str(output_dir),
            },
            indent=2,
        )
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
