...
\end{equation}
with \(T\) in kelvin and a mole-fraction activity convention.  \Cref{tab:reaction-equilibrium-constants} gives the source range of each coefficient correlation.  Evaluations outside those ranges test the fixed reaction-constant basis but supply no new reaction-constant estimate.  The ideal baseline replaces each activity coefficient with unity.
