...
from MEA.common.config import CANONICAL_MEA_WEIGHT_FRACTION, JOU_TEMPERATURES_C, LEGACY_BASELINE_OUT, REPO_ROOT
...
OUT_DIR = REPO_ROOT / "out" / "epcsaft" / "neutral_parity"
...
def plot_parity(curves: pd.DataFrame) -> Path:
    ...
    for temperature_C in TEMPERATURES_C:
        ...
        ax.plot(t_curve["CO2_loading"], t_curve["legacy_pcsaft_CO2_pressure_kPa"], ":", color=color, alpha=0.65)
        ax.plot(t_curve["CO2_loading"], t_curve["epcsaft_CO2_pressure_kPa"], "-", color=color, label=f"{temperature_C} C ePC-SAFT neutral")
