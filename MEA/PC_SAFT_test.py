...
from Get_True_Mol_Frac_New_all_species import solve_all_species_series
...
try:
    ...
    def flashTQ(t, q, x, params):
        ...
        bubble_p, y = local_pcsaft_core.pcsaft_bubbleP(
            ...
            dipm=np.asarray(params.get("dipm", np.zeros_like(x)), dtype=float),
            dip_num=np.asarray(params.get("dip_num", np.zeros_like(x)), dtype=float),
            z=np.asarray(params.get("z", np.zeros_like(x)), dtype=float),
        )
