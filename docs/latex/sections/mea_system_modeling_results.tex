...
On the 31 pressure records solved by both models, the fixed SSM+DS activity calculation is less accurate than the ideal-activity baseline.  It improves 4 records and worsens 27, with median absolute \(\log_{10}\) pressure residuals of 0.495 and 0.160, respectively.  The activity calculation converges across all 161 pressure records and 74 NMR states, but its closest speciation agreement occurs for the amine family; bicarbonate, carbonate, hydronium, and hydroxide remain weakly represented or weakly observed.
...
For positive measured species and aggregate values, median absolute \(\log_{10}(\mathrm{model}/\mathrm{data})\) residuals are 0.074 for \MEA, 0.033 for \MEAH, 0.040 for \MEACOO, 0.461 for \HCOthree, 0.599 for \COthree, and 0.066 for \ce{MEA + MEAH+}.  At the 15 states where the source reports zero \HCOthree, the largest modeled mole fraction is 0.00128.  These values are upper-bound checks because the sources give no detection limits.
...
The controlled pressure comparison uses the same 31 Jou et al.\ measurements for both models.  The ideal baseline gives median absolute, signed median, and RMSE \(\log_{10}(P_{\mathrm{model}}/P_{\mathrm{data}})\) residuals of 0.160, \(-0.024\), and 0.297, respectively; the activity model gives 0.495, 0.226, and 0.565.  The activity model improves 4 paired records and worsens 27.  The fixed activity parameter set therefore reduces pressure accuracy on this subset.  This result diagnoses the transferred parameter set, not the general ePC-SAFT formulation.
...
\Cref{tab:literature-model-comparison} distinguishes apparent-chemistry PC-SAFT treatments, explicit-ion ePC-SAFT amine models, and advanced-Born electrolyte models for gas solubility.  The present calculation applies explicit-ion activity coupling and SSM+DS modified-Born solvation to carbamate-forming \MEA.  Following prior ePC-SAFT amine-solubility studies, we separate parameter provenance from VLE evaluation and add measured \MEA speciation residuals to the pressure comparison~\cite{Uyan2015,Wangler2018}.

\subsection{Model limitations and transferability}
The parameter values predate this analysis and lack a reproducible estimation history, so the source-stratified residuals measure fixed-set transfer rather than held-out prediction.  A local finite-difference screen is most sensitive to \(k_{\MEA,\HtwoO}\), \(k_{\COtwo,\MEA}\), and \(d_{\COthree}^{\mathrm{Born}}\); \(d_{\HthreeO}^{\mathrm{Born}}\) has little effect on the reported metrics.  Future regression should jointly fit pressure, measured speciation, and reaction-equilibrium constraints, with parameter bounds and independent validation data declared before fitting.
