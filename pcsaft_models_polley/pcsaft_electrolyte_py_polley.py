...
def f_dielc(x, dielc):
    ...
    for i in range(len(x)):
        dielc_sum += dielc[i] * x[i]

    return dielc_sum


def pcsaft_vaporP(p_guess, t, x, params):
    """
    ...
    result = minimize(vaporPfit, p_guess, args=(t, x, params), tol=1e-10, method='Nelder-Mead',
                      options={'maxiter': 100})
...
def pcsaft_bubbleP(p_guess, xv_guess, t, x, params):
    """
    ...
    """
    check_input(x, 'guess pressure', p_guess, 'temperature', t)
    result = minimize(bubblePfit, p_guess, args=(xv_guess, t, x, params), tol=1e-10, method='Nelder-Mead',
                      options={'maxiter': 100})
    ...
    if not (
            'z' in params):  # Check that the mixture does not contain electrolytes. For electrolytes, a different equilibrium criterion should be used.
        rho = pcsaft_den(t, x, bubP, params, phase='liq')
        fugcoef_l = pcsaft_fugcoef(t, rho, x, params)
        ...
        while (dif > 1e-9) and (itr < 100):
            xv_old[:] = xv
            rho = pcsaft_den(t, xv, bubP, params, phase='vap')
            fugcoef_v = pcsaft_fugcoef(t, xv, rho, params)
        ...
        while (dif > 1e-9) and (itr < 100):
            ...
            xv[np.where(z == 0)[0]] = (fugcoef_l * x / fugcoef_v)[
                np.where(z == 0)[0]]  # here it is assumed that the ionic compounds are nonvolatile
            xv = xv / np.sum(xv)
            dif = np.sum(abs(xv - xv_old))
            itr += 1
...
def pcsaft_Hvap(p_guess, t, x, params):
    """
    ...
    """
    check_input(x, 'guess pressure', p_guess, 'temperature', t)
    Pvap = minimize(vaporPfit, p_guess, args=(t, x, params), tol=1e-10, method='Nelder-Mead',
                    options={'maxiter': 100}).x

    rho = pcsaft_den(t, x, Pvap, params, phase='liq')
    hres_l = pcsaft_hres(t, rho, x, params)
    rho = pcsaft_den(t, x, Pvap, params, phase='vap')
    hres_v = pcsaft_hres(t, rho, x, params)
    Hvap = hres_v - hres_l
...
def pcsaft_osmoticC(t, rho, x, params):
    ...
    m, s, e = params['m'], params['s'], params['e']
    k_ij, k_hb, e_assoc, vol_a, assoc_scheme = params['k_ij'], params['k_hb'], params['e_assoc'], params['vol_a'], \
        params['assoc_scheme']
    dipm, dip_num = params['dipm'], params['dip_num']
    z, dielc = params['z'], f_dielc(x, params['dielc'])
    ...
    fugcoef = pcsaft_fugcoef(t, rho, x, params)
    p = pcsaft_p(t, rho, x, params)
    if rho < 900:
        ph = 'vap'
    else:
        ph = 'liq'
    rho0 = pcsaft_den(t, x0, p, params, phase=ph)
    fugcoef0 = pcsaft_fugcoef(t, x0, rho0, params)
...
def pcsaft_cp(t, rho, x, params):
    ...
    p = pcsaft_p(t, rho, x, params)
    rho0 = pcsaft_den(t - 0.001, x, p, params, phase=ph)
    hres0 = pcsaft_hres(t - 0.001, x, rho0, params)
    rho1 = pcsaft_den(t + 0.001, x, p, params, phase=ph)
    hres1 = pcsaft_hres(t + 0.001, x, rho1, params)
    dhdt = (hres1 - hres0) / 0.002  # a numerical derivative is used for now until analytical derivatives are ready
    return cp_ideal + dhdt


def pcsaft_PTz(p_guess, x_guess, beta_guess, mol, vol, x_total, t, params):
    """
    ...
    result = minimize(PTzfit, p_guess, args=(x_guess, beta_guess, mol, vol, t, x_total, params), tol=1e-10,
    ...
    if not (
        ...
        while (dif > 1e-9) and (itr < 100):
            beta_old = beta
            rhol = pcsaft_den(t, xl, p, params, phase='liq')
            fugcoef_l = pcsaft_fugcoef(t, xl, rhol, params)
            rhov = pcsaft_den(t, xv, p, params, phase='vap')
            fugcoef_v = pcsaft_fugcoef(t, xv, rhov, params)
            if beta > 0.5:
                xl = fugcoef_v * xv / fugcoef_l
                xl = xl / np.sum(xl)
                xv = (mol * x_total - (
                        1 - beta) * mol * xl) / beta / mol  # if beta is close to zero then this equation behaves poorly, and that is why we use this if statement to switch the equation around
            else:
        ...
        while (dif > 1e-9) and (itr < 100):
            ...
            if beta > 0.5:
                xl = fugcoef_v * xv / fugcoef_l
                xl = xl / np.sum(xl) * (((1 - beta) - np.sum(x_total[np.where(z != 0)[0]])) / (
                        1 - beta))  # ensures that mole fractions add up to 1
...
def pcsaft_den(t, p, x, params, phase='liq'):
    ...
    rho = root(denfit, rho_guess, args=(t, p, x, params)).x[0]
...
def pcsaft_dadt(t, rho, x, params):
    ...
    if type(z) == np.ndarray:
        d[np.where(z != 0)[0]] = s[np.where(z != 0)[0]] * (
                1 - 0.12)  # for ions the diameter is assumed to be temperature independent (see Held et al. 2014)
    ...
    for i in range(ncomp):
        for j in range(ncomp):
            ...
            if type(z) == np.ndarray:
                if z[i] * z[
                    j] <= 0:  # for two cations or two anions e_ij is kept at zero to avoid dispersion between like ions (see Held et al. 2014)
    ...
    dadt_disp = -2 * np.pi * den * (dI1_dt - I1 / t) * m2es3 - np.pi * den * m_avg * (
            dC1_dt * I2 + C1 * dI2_dt - 2 * C1 * I2 / t) * m2e2s3

    # Dipole term (Gross and Vrabec term) --------------------------------------
    if type(dipm) != np.ndarray or np.sum(dipm) == 0.0:
        dadt_polar = 0
    else:
    ...
    if type(e_assoc) != np.ndarray:
        ...
        for i in range(ncA):
            for j in range(ncA):
                eABij[i, j] = (e_assoc[iA[i]] + e_assoc[iA[j]]) / 2. * (1 - k_hb[i, j])
                volABij[i, j] = np.sqrt(vol_a[iA[i]] * vol_a[iA[j]]) * (np.sqrt(s_ij[iA[i], iA[i]] \
                                                                                * s_ij[iA[j], iA[j]]) / (0.5 * (
                        s_ij[iA[i], iA[i]] + s_ij[iA[j], iA[j]]))) ** 3
                delta_ij[i, j] = ghs[iA[i], iA[j]] * (np.exp(eABij[i, j] / t) - 1) * s_ij[iA[i], iA[j]] ** 3 * volABij[
                    i, j]
                XA[i, :] = (-1 + np.sqrt(1 + 8 * den * delta_ij[i, i])) / (4 * den * delta_ij[i, i])
                ddelta_dt[i, j] = s_ij[iA[j], iA[j]] ** 3 * volABij[i, j] * (-eABij[i, j] / t ** 2 \
                                                                             * np.exp(eABij[i, j] / t) * ghs[
                                                                                 iA[i], iA[j]] + dghs_dt[iA[i], iA[j]] \
    ...
    if len(z) == 0:
        ...
        kappa = np.sqrt(den * E_CHRG ** 2 / kb / t / (dielc * perm_vac) * np.sum(
            z ** 2 * x))  # the inverse Debye screening length. Equation 4 in Held et al. 2008.

        if kappa == 0:
            dadt_ion = 0
        else:
            dkappa_dt = -0.5 * den * E_CHRG ** 2 / kb / t ** 2 / (dielc * perm_vac) * np.sum(z ** 2 * x) / kappa
            chi = 3 / (kappa * d) ** 3 * (1.5 + np.log(1 + kappa * d) - 2 * (1 + kappa * d) + \
                                          0.5 * (1 + kappa * d) ** 2)
            dchikap_dk = -2 * chi + 3 / (1 + kappa * d)
...
def pcsaft_hres(t, rho, x, params):
    ...
    Z = pcsaft_Z(t, rho, x, params)
    dares_dt = pcsaft_dadt(t, rho, x, params)
...
def pcsaft_sres(t, rho, x, params):
    ...
    gres = pcsaft_gres(t, rho, x, params)
    hres = pcsaft_hres(t, rho, x, params)
...
def pcsaft_gres(t, rho, x, params):
    ...
    ares = pcsaft_ares(t, rho, x, params)
    Z = pcsaft_Z(t, rho, x, params)
...
def pcsaft_lnfugcoef(t, rho, x, params):
    ...
    for i in range(ncomp):
        for j in range(ncomp):
            ...
            ghs[i, j] = 1 / (1 - zeta[3]) + (d[i] * d[j] / (d[i] + d[j])) * 3 * zeta[2] / (1 - zeta[3]) ** 2 + \
                        (d[i] * d[j] / (d[i] + d[j])) ** 2 * 2 * zeta[2] ** 2 / (1 - zeta[3]) ** 3
            denghs[i, j] = zeta[3] / (1 - zeta[3]) ** 2 + (d[i] * d[j] / (d[i] + d[j])) * (
                    3 * zeta[2] / (1 - zeta[3]) ** 2 + \
                    6 * zeta[2] * zeta[3] / (1 - zeta[3]) ** 3) + (d[i] * d[j] / (d[i] + d[j])) ** 2 * (
                                   4 * zeta[2] ** 2 / (1 - zeta[3]) ** 3 + \
        ...
        for j in range(ncomp):
            dghs_dx[j, i] = dzeta_dx[3] / (1 - zeta[3]) ** 2 + (d[j] * d[j] / (d[j] + d[j])) * \
                            ...
                            + (d[j] * d[j] / (d[j] + d[j])) ** 2 * (4 * zeta[2] * dzeta_dx[2] / (1 - zeta[3]) ** 3 \
                                                                    + 6 * zeta[2] ** 2 * dzeta_dx[3] / (
                                                                            1 - zeta[3]) ** 4)
        dahs_dx[i] = -dzeta_dx[0] / zeta[0] * ares_hs + 1 / zeta[0] * (3 * (dzeta_dx[1] * zeta[2] \
                                                                            + zeta[1] * dzeta_dx[2]) / (
                                                                               1 - zeta[3]) + 3 * zeta[1] * zeta[
                                                                           2] * dzeta_dx[3] \
                                                                       / (1 - zeta[3]) ** 2 + 3 * zeta[2] ** 2 *
                                                                       dzeta_dx[2] / zeta[3] / (1 - zeta[3]) ** 2 \
                                                                       + zeta[2] ** 3 * dzeta_dx[3] * (
                                                                               3 * zeta[3] - 1) / zeta[3] ** 2 / (
                                                                               1 - zeta[3]) ** 3 \
                                                                       + np.log(1 - zeta[3]) * ((3 * zeta[2] ** 2 *
                                                                                                 dzeta_dx[2] * zeta[3] - \
                                                                                                 2 * zeta[2] ** 3 *
                                                                                                 dzeta_dx[3]) / zeta[
                                                                                                    3] ** 3 - dzeta_dx[
                                                                                                    0]) + \
                                                                       (zeta[0] - zeta[2] ** 3 / zeta[3] ** 2) *
                                                                       dzeta_dx[3] / (1 - zeta[3]))
    ...
    for i in range(ncomp):
        ...
        dC1_dx = C2 * dzeta3_dx - C1 ** 2 * (m[i] * (8 * eta - 2 * eta ** 2) / (1 - eta) ** 4 - \
                                             m[i] * (20 * eta - 27 * eta ** 2 + 12 * eta ** 3 - 2 * eta ** 4) / (
                                                     (1 - eta) * (2 - eta)) ** 2)
        ...
        for i in range(ncomp):
            for j in range(ncomp):
                ...
                dA2_det += x[i] * x[j] * e_ij[i, i] / t * e_ij[j, j] / t * s_ij[i, i] ** 3 \
                           * s_ij[j, j] ** 3 / s_ij[i, j] ** 3 * dip_num[i] * dip_num[j] * dipmSQ[i] * dipmSQ[
                               j] * dJ2_det
                if i == j:
    ...
    if type(e_assoc) != np.ndarray:
        ...
        a_sites = 2
        iA = np.nonzero(e_assoc)[0]  #indices of associating compounds
        iA = np.array([i for i, val in enumerate(assoc_scheme) if val is not None])
        ncA = iA.shape[0]  # number of associating compounds in the fluid
            ...
            for j in range(ncA):
                ...
                for k in range(ncomp):
                    dghsd_dd = np.pi / 6. * m[k] * (d[k] ** 3 / (1 - zeta[3]) ** 2 + 3 * d[iA[i]] * d[iA[j]] \
                                                    / (d[iA[i]] + d[iA[j]]) * (
                                                            d[k] ** 2 / (1 - zeta[3]) ** 2 + 2 * d[k] ** 3 * \
                                                            zeta[2] / (1 - zeta[3]) ** 3) + 2 * (
                                                            d[iA[i]] * d[iA[j]] / (d[iA[i]] + d[iA[j]])) ** 2 * \
                                                    (2 * d[k] ** 2 * zeta[2] / (1 - zeta[3]) ** 3 + 3 * d[k] ** 3 *
                                                     zeta[2] ** 2 \
                                                     / (1 - zeta[3]) ** 4))
                    ddelta_dd[i, j, k] = dghsd_dd * (np.exp(eABij[i, j] / t) - 1) * s_ij[iA[i], iA[j]] ** 3 * volABij[
                        i, j]
            XA[i, :] = (-1 + np.sqrt(1 + 8 * den * delta_ij[i, i])) / (4 * den * delta_ij[i, i])
    ...
    if len(z) == 0:
        ...
        if kappa != 0:
            chi = 3 / (kappa * d) ** 3 * (
                    1.5 + np.log(1 + kappa * d) - 2 * (1 + kappa * d) + 0.5 * (1 + kappa * d) ** 2)
    ...
    if len(z) == 0:
        ...
        mu_born = np.zeros_like(x)
        da_born_dx = np.zeros_like(x)
        ddielc_dx = np.zeros_like(x)
        E_CHRG = 1.6021766208e-19  # elementary charge, units of coulomb
        perm_vac = 8.854187817e-22  #permittivity in vacuum, C V^-1 Angstrom^-1
        ...
        ares_born = -E_CHRG ** 2 / (4. * np.pi * kb * t * perm_vac) * (1 - 1 / dielc) * np.sum((x * z ** 2 / a))
        Z_born = 0
        ddielc_dx = params['dielc']

        for i in range(ncomp):
            da_born_dx[i] = - E_CHRG ** 2 / (4. * np.pi * kb * t * perm_vac) * ((1 - 1 / dielc) * (z[i] ** 2 / a[i]) + np.sum((x * z ** 2 / a)) * (1 / dielc ** 2) * (ddielc_dx[i]))

        for i in range(ncomp):
            mu_born[i] = ares_born + Z_born + da_born_dx[i] - np.sum(x * da_born_dx)

    mu = mu_hc + mu_disp + mu_polar + mu_assoc + mu_ion + mu_born

    Z = pcsaft_Z(t, rho, x, params)

    lnfugcoef = mu - np.log(Z)  # the fugacity coefficients
...
def pcsaft_fugcoef(t, rho, x, params):
    ...
    return np.exp(pcsaft_lnfugcoef(t, rho, x, params))
...
def pcsaft_Z(t, rho, x, params):
    ...
    if type(e_assoc) != np.ndarray:
        ...
        for i in range(ncA):
            for j in range(ncA):
                eABij[i, j] = (e_assoc[iA[i]] + e_assoc[iA[j]]) / 2. * (1 - k_hb[i, j])
                volABij[i, j] = (np.sqrt(vol_a[iA[i]] * vol_a[iA[j]]) *
                                 (np.sqrt(s_ij[iA[i], iA[i]] * s_ij[iA[j], iA[j]]) /
                                  (0.5 * (s_ij[iA[i], iA[i]] + s_ij[iA[j], iA[j]]))) ** 3)
                delta_ij[i, j] = ghs[iA[i], iA[j]] * (np.exp(eABij[i, j] / t) - 1) * s_ij[iA[i], iA[j]] ** 3 * volABij[
                    i, j]
                for k in range(ncomp):
                    dghsd_dd = np.pi / 6. * m[k] * (d[k] ** 3 / (1 - zeta[3]) ** 2 + 3 * d[iA[i]] * d[iA[j]] \
    ...
    Z = Zid + Zhc + Zdisp + Zpolar + Zassoc + Zion + Zborn
...
def pcsaft_ares(t, rho, x, params):
    ...
    if type(e_assoc) != np.ndarray:
        ...
        dif = 1000.
        XA_old = np.copy(XA)
        while (ctr < 100) and (dif > 1e-15):
            ctr += 1
            XA = XA_find(XA_old, ncA, delta_ij, den, x[iA])
            dif = np.sum(abs(XA - XA_old))
            XA_old[:] = (XA + XA_old) / 2.0
        ...
        for i in range(ncomp):
            for j in range(ncomp):
                for k in range(ncomp):
                    ...
                    A3 += x[i] * x[j] * x[k] * (e_ij[i, i] / t) * (e_ij[j, j] / t) * (e_ij[k, k] / t) * \
                          (s_ij[i, i] ** 3 * s_ij[j, j] ** 3 * s_ij[k, k] ** 3 / (s_ij[i, j] * s_ij[i, k] \
                                                                                  * s_ij[j, k])) * dip_num[i] * dip_num[
                              j] * dip_num[k] * dipmSQ[i] \
                          * dipmSQ[j] * dipmSQ[k] * J3
    ...
    if len(z) == 0:
        ...
        ares_born = -E_CHRG ** 2 / (4. * np.pi * kb * t * perm_vac) * (1 - 1 / dielc) * np.sum(x * z ** 2 / d)

    print(
        f'hc: {ares_hc:.8f}, disp: {ares_disp:.8f}, polar: {ares_polar:.8f}, assoc: {ares_assoc:.8f}, ion: {ares_ion:.8f}')

    ares = ares_hc + ares_disp + ares_polar + ares_assoc + ares_ion + ares_born
...
def XA_find(XA_guess, ncomp, delta_ij, den, x):
    ...
    n_sites = math.ceil(XA_guess.shape[1] / ncomp)
...
def bubblePfit(p_guess, xv_guess, t, x, params):
    """Minimize this function to calculate the bubble point pressure."""
    if not (
            'z' in params):  # Check that the mixture does not contain electrolytes. For electrolytes, a different equilibrium criterion should be used.
        rho = pcsaft_den(t, x, p_guess, params, phase='liq')
        fugcoef_l = pcsaft_fugcoef(t, rho, x, params)
        ...
        while (dif > 1e-9) and (itr < 100):
            xv_old[:] = xv
            rho = pcsaft_den(t, xv, p_guess, params, phase='vap')
            fugcoef_v = pcsaft_fugcoef(t, xv, rho, params)
            xv = fugcoef_l * x / fugcoef_v
            xv = xv / np.sum(xv)
            dif = np.sum(abs(xv - xv_old))
        ...
        while (dif > 1e-9) and (itr < 100):
            xv_old[:] = xv
            rho = pcsaft_den(t, xv_guess, p_guess, params, phase='vap')
            fugcoef_v = pcsaft_fugcoef(t, xv_guess, rho, params)
        ...
        error = np.sum(error[np.where(z == 0)[
            0]])  # again, ionic components are considered to be nonvolatile and are not taken into account
...
def vaporPfit(p_guess, t, x, params):
    ...
    if p_guess <= 0:
        ...
        rho = pcsaft_den(t, x, p_guess, params, phase='liq')
        fugcoef_l = pcsaft_fugcoef(t, rho, x, params)
        rho = pcsaft_den(t, x, p_guess, params, phase='vap')
        fugcoef_v = pcsaft_fugcoef(t, rho, x, params)
...
def denfit(rho_guess, t, p, x, params):
    """Minimize this function to calculate the density."""
    P_fit = pcsaft_p(t, rho_guess, x, params)
    
    return np.sum(((P_fit - p) / p * 100) ** 2)


def PTzfit(p_guess, x_guess, beta_guess, mol, vol, x_total, t, params):
    ...
    if not (
        ...
        while (dif > 1e-9) and (itr < 100):
            beta_old = beta
            rhol = pcsaft_den(t, xl, p_guess, params, phase='liq')
            fugcoef_l = pcsaft_fugcoef(t, xl, rhol, params)
            rhov = pcsaft_den(t, xv, p_guess, params, phase='vap')
            fugcoef_v = pcsaft_fugcoef(t, xv, rhov, params)
            if beta > 0.5:
                xl = fugcoef_v * xv / fugcoef_l
                xl = xl / np.sum(xl)
...
def pcsaft_fit_pure(params, prop, T, P, molden):
    """
    ...
    for i in range(prop.shape[0]):
        if prop[i] == 0:
            if molden[i] < 900:
                error[i] = (pcsaft_den(x, np.asarray([params[0]]), np.asarray([params[1]]), np.asarray([params[2]]),
                                       T[i], P[i], params, phase='vap') - molden[i]) / molden[i] * 100
            else:
                error[i] = (pcsaft_den(x, np.asarray([params[0]]), np.asarray([params[1]]), np.asarray([params[2]]),
                                       T[i], P[i], params) - molden[i]) / molden[i] * 100
        elif prop[i] == 1:
            P_fit = pcsaft_vaporP(P[i], x, np.asarray([params[0]]), np.asarray([params[1]]), np.asarray([params[2]]),
                                  T[i], params)
