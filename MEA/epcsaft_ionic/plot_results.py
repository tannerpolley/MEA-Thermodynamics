...
def plot_pressure(rows: list[dict[str, object]]):
    ...
    colors = {40: "tab:orange", 60: "tab:blue", 80: "tab:green", 100: "tab:red", 120: "tab:purple"}
    for temperature_C in JOU_TEMPERATURES_C:
        ...
        if not data.empty:
            ax.plot(data["CO2_loading"], data["CO2_pressure"], "x", color=color)
        ...
        if not ok.empty:
            ...
            ax.plot(ok["CO2_loading"], ok["raw_pred_CO2_pressure_kPa"], "-", color=color, label=f"{temperature_C} C, med |log10 err|={med:.2f}")
...
def plot_speciation(rows: list[dict[str, object]]):
    ...
    for species, color in {
        ...
        if target_column in subset:
            ax.semilogy(subset["CO2_loading"], subset[target_column], "o", color=color, alpha=0.35)
        if model_column in subset:
            ax.semilogy(subset["CO2_loading"], subset[model_column], "-", color=color, label=species)
    ax.set_xlabel("CO2 loading, mol CO2/mol MEA")
    ax.set_ylabel("True-species mole fraction")
    ax.set_title("Full ionic ePC-SAFT activity-coupled speciation")
    ax.legend()
    fig.tight_layout()
...
def main() -> int:
    ...
    pressure_csv = write_csv(OUT_DIR / "ionic_pressure_comparison.csv", p_rows)
    speciation_csv = write_csv(OUT_DIR / "ionic_speciation_activity_residuals.csv", s_rows)
    ...
    write_json(OUT_DIR / "ionic_evaluation_summary.json", summary)
