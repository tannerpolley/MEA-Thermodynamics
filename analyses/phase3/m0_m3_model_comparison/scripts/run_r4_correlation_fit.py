...
import numpy as np
import run_polar_comparison as polar
...
TEMPERATURE_RANGE_K = (313.15, 393.15)
PRESSURE_RANGE_PA = (6105.45, 3_000_000.0)
...
INITIAL_PREFLIGHT_FAILURES = {
    "vle_obs_0126": {
        "failure_kind": "physical_domain_failure",
        "failure_reason": "first failed physical criterion=trace_status; search status=domain_rejected",
    },
    "vle_obs_0134": {
        "failure_kind": "physical_domain_failure",
        "failure_reason": "first failed physical criterion=trace_status; search status=domain_rejected",
    ...
    "vle_obs_0233": {
        "failure_kind": "provider_domain_rejection",
        "failure_reason": "requested density branch is unavailable",
    },
}
EOS_PARAMETER_SPECS = (
    {
...
def _prepare_fit_bundle(destination: Path) -> None:
    polar._prepare_bundle(destination, "M5")
...
def _public_transfer_offsets(
    ...
    declaration = epcsaft.SourceReferenceDeclaration(
        ...
        solvent_mole_fractions=tuple(
            1.0 if component_id == "water" else 0.0 for component_id in component_ids
        ),
        activity_convention_id="molality-infinite-dilution-v1",
...
def main() -> None:
    ...
    fit_training_rows = tuple(
        ...
        if row.observation_id not in INITIAL_PREFLIGHT_FAILURES
    )
    ...
    with tempfile.TemporaryDirectory(prefix="mea-r4-fit-") as temporary:
        ...
        initial_training_results = _evaluate_batch(
            bundle,
            fit_training_rows,
            literature_coefficients,
            offsets,
        ...
        evaluable_rows = tuple(
            row for row in rows if row.observation_id not in INITIAL_PREFLIGHT_FAILURES
        )
        ...
        failed_ids = set(INITIAL_PREFLIGHT_FAILURES)
        failed_ids.update(
            str(result["observation_id"])
            for result in (*initial_reserved_failures, *candidate_failures)
        )
        sensitivity_rows = []
    ...
    for model_id in ("M5_literature_R4", "M5_fitted_R4"):
        for observation_id, failure in INITIAL_PREFLIGHT_FAILURES.items():
            row = row_lookup[observation_id]
            state_failure_rows.append(
                {
                    ...
                    "evidence_stage": "final-wheel literature-R4 all-training preflight",
                }
            )
    for model_id, failures in (
        ("M5_literature_R4", initial_reserved_failures),
            ...
            fit_rows.append(
                {
                    ...
                    **{
                        f"dlog10_pco2_dlnk_{reaction_id}": result[
                            f"dlog10_pco2_dlnk_{reaction_id}"
                        ]
                        for reaction_id in REACTION_IDS
                    },
                    **{
                        f"dlog10_meacoo_dlnk_{reaction_id}": result[
                            f"dlog10_meacoo_dlnk_{reaction_id}"
                        ]
                        for reaction_id in REACTION_IDS
                    },
                }
    ...
    _write_csv(RESULTS / "r4_correlation_fit_iterations.csv", iterations)
    ...
    receipt = {
        ...
        "row_counts": {
            ...
            "training_preflight_failed": len(INITIAL_PREFLIGHT_FAILURES),
        ...
        "source_files": {
            ...
            for path in (VLE, SPECIATION, METROLOGY, SPLIT)
        },
        "installed_artifacts": {
